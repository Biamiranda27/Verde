...
class Testar{
    public boolean p(String pal){
      return p(pal, 0, pal.length() -1);
}
public boolean p(String pal, int esquerda, int direita){
    if(esquerda >= direita){
        return true;
    } if(pal.charAt(esquerda) != pal.charAt(direita)){
        return false;
    }
    return p(pal, esquerda +1, direita-1);
 }
}
